...
namespace Practico
{
    public class GameMngr
    {
        ...
        private void PlayStage() {
            ...
            while (bb.KeyDown(bb.KEY_ESCAPE) == 0 && !game.bomberman_is_dead && !game.end_time && !game.level_finished)
            {
                UpdateStage();
            ...
            if (game.level_finished) {
                game.actual_score += game.actual_time;
                game.max_stage++;
                pressedPlay = false;
                pressedEdit = false;
            }
        ...
        private void exploteBomb() {
            ...
            if (game.actualStage.bombs.Count > 0){
                ...
                if (b != null && (bb.KeyHit(bb.KEY_SPACE) == 1 || b.time == 0)){
                    ...
                    int explosion = bb.LoadSprite("Images//Explosion//exp_25.png", 4);
                    bb.EntityAlpha(explosion, 0.5f);
        ...
        /*private void drawExplosion() {
            ...
            if (now.Subtract(this.bombs_lag).TotalMilliseconds >= 20)
            {
                this.bombs_lag = DateTime.Now;
                foreach (Bomb b in game.actualStage.bombs)
                {
                    if (b.time == 0) {
                        graphicMngr.drawExplosion(b);
                        //graphicMngr.removeExplosion(b);
                        b.actual_frame++;
                    }
                }
        ...
        {
            DateTime now = DateTime.Now;
            if (now.Subtract(this.enemies_lag).TotalMilliseconds >= 100)
            {
